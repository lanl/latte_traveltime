...
opts = "-n1=101 -d1=0.01 -n2=201 -d2=0.01 -d3=0.01 -label1='Depth (km)' -label2='Y (km)' -label3='X (km)' -tick1d=0.25 -tick2d=0.5 -tick3d=1.0 -mtick1=4 -mtick2=4 -mtick3=9 -color=rainbowcmyk -legend=y -size1=3 -size2=4 -size2=5 -slice1=0.15 -slice2=1.1 -slice3=1.6 -lticksize=#{fsize} -unitsize=#{fsize} -tick1size=#{fsize} -tick2size=#{fsize} -tick3size=#{fsize} -label1size=#{fsize} -label2size=#{fsize} -label3size=#{fsize} "
...
# true
system "x_showslice -tr=tloc_fault/gt_fault_sxyz_1.png -in=model/vp.bin #{opts} -cmin=#{pmin} -cmax=#{pmax} -unit='P-wave Velocity (m/s)' -ld=500 -lmtick=4 -out=tloc_fault/gt_vp.pdf &"
system "x_showslice -tr=tloc_fault/gt_fault_sxyz_2.png -in=model/vp_init.bin #{opts} -cmin=#{pmin} -cmax=#{pmax} -unit='P-wave Velocity (m/s)' -ld=500 -lmtick=4 -out=tloc_fault/init_vp.pdf &"
